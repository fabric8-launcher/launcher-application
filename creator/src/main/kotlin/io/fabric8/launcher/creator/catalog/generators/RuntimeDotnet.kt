...
interface RuntimeDotnetProps : LanguageCsharpProps {
    val runtime: Runtime
    ...
    open class Data(map: Properties = propsOf()) : LanguageCsharpProps.Data(map), RuntimeDotnetProps {
        override val runtime: Runtime by _map
        override val dotnet: DotnetCoords by _map
    }
}

class RuntimeDotnet(ctx: CatalogItemContext) : BaseGenerator(ctx) {
    override fun apply(resources: Resources, props: Properties, extra: Properties): Resources {
        val pdprops = RuntimeDotnetProps.build(props)
