...
public class LocalStatusMessageEventBrokerTest {
    ...
    private StatusMessageEvent githubWebhookEvent(UUID key) {
        return new StatusMessageEvent(key, LauncherStatusEventType.GITHUB_WEBHOOK);
    }

    @NotNull
    private StatusMessageEvent githubPushEvent(UUID key) {
        return new StatusMessageEvent(key, LauncherStatusEventType.GITHUB_PUSHED);
    }

    @NotNull
    private StatusMessageEvent githubCreateEvent(UUID key) {
        return new StatusMessageEvent(key, LauncherStatusEventType.GITHUB_CREATE);
    }
