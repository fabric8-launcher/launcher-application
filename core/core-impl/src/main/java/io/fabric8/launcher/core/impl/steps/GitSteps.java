...
import static io.fabric8.launcher.core.api.events.LauncherStatusEventType.GITHUB_CREATE;
...
public class GitSteps {
    ...
    public GitRepository createGitRepository(CreateProjectile projectile) {
        ...
        if (projectile.getStartOfStep() > LauncherStatusEventType.GITHUB_CREATE.ordinal()) {
    ...
    public void pushToGitRepository(CreateProjectile projectile, GitRepository repository) {
        if (projectile.getStartOfStep() <= LauncherStatusEventType.GITHUB_PUSHED.ordinal()) {
